...
## 2026-02-22 — Итеративный редизайн топбара для Novikov

### Топбар и визуальная консистентность
- Топбар переведен в формат «контейнер-карточки»:
  - граница: `1px solid var(--stroke)`
  - `border-radius: var(--radius)`
  - прозрачный фон `rgba(255,255,255,0.85)`
  - для темной темы: `rgba(30,30,30,0.85)`
- Выровнены внешние границы топбара и карточек вин:
  - унифицирован внутренний горизонтальный отступ (`18px`) в контейнере
  - добавлен соответствующий внутренний отступ для контейнера сетки

### Навигация меню и компоновка
- Реализована единая навигация верхнего уровня на базе `config.menus`.
- Для `novikov_bh` зафиксирован набор меню:
  - Bar
  - Coffee
  - Pastry
  - Wine
- Меню перемещено в правую группу топбара рядом с поиском и настройками.

### Поиск и настройки
- Поиск и кнопка настроек объединены в одну визуальную группу справа.
- Обновлена геометрия контролов:
  - высота меню/поиска/настроек унифицирована до `36px`
  - ширина поиска зафиксирована на `96px`
  - скругление контролов унифицировано через `var(--radius)`
- Иконка настроек заменена на более лаконичный символ `⋮`.
- Поиск восстановлен после визуальных экспериментов и работает корректно.

### Типографика и брендинг
- Приведен к единому виду размер текста в меню/поиске/настройках: `16px`.
- Приведены веса шрифтов:
  - меню/поиск/настройки: `450`
  - активный пункт меню: `500`
- Логотип в топбаре обновлен до `60×60` без изменения остальной структуры.
